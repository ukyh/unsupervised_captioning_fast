...
tf.flags.DEFINE_string('selfcap', 'selfcap_ss4_mi2.tfrec', 'self caption path')
...
def input_fn(batch_size):
  image_ds = tf.data.TFRecordDataset('data/image_selftrain.tfrec')
  ...
  # NOTE: Sentences loaded here are w/o {B, E}OS.
  #       {B, E}OS is added in `dataset.map(take)`
  sentence_ds = tf.data.TFRecordDataset('data/' + FLAGS.selfcap)
  sentence_ds = sentence_ds.map(parse_sentence, num_parallel_calls=AUTOTUNE)

  dataset = tf.data.Dataset.zip((image_ds, sentence_ds))
  dataset = dataset.filter(lambda im, sen: tf.greater(sen[3], 1))
